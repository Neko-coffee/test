...
def snow_sim(pointcloud,severity):
    ...
    # 🔥 确保返回 float32 类型（MMDetection3D 要求）
    # 注意：monte_carlo_augment 已经返回 float32，但这里再确认一次
...
# 🔥 创建别名以匹配 test_time_aug.py 中的导入
rain_sim_lidar = rain_sim
fog_sim_lidar = fog_sim
snow_sim_lidar = snow_sim
filter_point_by_angle = fov_filter
