# 🔥🔥🔥 必须在所有导入之前修复 scipy.integrate.trapz（PyMieScatt 在导入时就会使用）
# 关键：必须在导入任何其他模块之前执行
import numpy as np
import sys
import importlib

# 立即导入并修补 scipy.integrate，确保 trapz 存在
# 方法1：直接导入并修补
import scipy.integrate
if not hasattr(scipy.integrate, 'trapz'):
    scipy.integrate.trapz = np.trapz
    # 更新 __all__ 列表
    if hasattr(scipy.integrate, '__all__') and 'trapz' not in scipy.integrate.__all__:
        scipy.integrate.__all__.append('trapz')

# 方法2：同时更新 sys.modules（确保所有引用都看到 trapz）
if 'scipy.integrate' in sys.modules:
    sys.modules['scipy.integrate'].trapz = np.trapz

# 方法3：确保 scipy.integrate 模块对象是同一个
import scipy
if hasattr(scipy, 'integrate') and not hasattr(scipy.integrate, 'trapz'):
    scipy.integrate.trapz = np.trapz
...
# 🔥 现在可以安全导入 PyMieScatt 了（scipy.integrate.trapz 已经修复）
try:
    import PyMieScatt as ps
except ImportError as e:
    if 'trapz' in str(e).lower():
        # 如果还是 trapz 的问题，进行最后一次尝试
        # 直接修改 PyMieScatt 可能需要的模块
        import scipy.integrate
        if not hasattr(scipy.integrate, 'trapz'):
            scipy.integrate.trapz = np.trapz
        # 重新尝试导入
...
# 🔥 导入 trapz（已从 scipy.integrate 修补或从 numpy 导入）
try:
    from scipy.integrate import trapz
except ImportError:
    # 如果 scipy.integrate 中没有 trapz（新版 scipy），使用 numpy 的版本
...
class LISA:
    ...
    def monte_carlo_augment(self, pc: np.ndarray, Rr: int, fixed_seed: bool = False) -> np.ndarray:
        ...
        """
        # 🔥 固定输出为 6 列：x, y, z, intensity, label, intensity_diff
        # 不管输入 pc 有多少列，输出都是 6 列
        pc_new = np.zeros((pc.shape[0], 6))
        ...
        # 🔥 检测是否在 daemon 进程中（如 DataLoader worker）
        # daemon 进程不能创建子进程，需要使用顺序处理
        try:
            is_daemon = mp.current_process().daemon
        except:
            is_daemon = False
        
        # 如果在 daemon 进程中或进程数量限制，使用顺序处理
        use_sequential = is_daemon or mp.cpu_count() <= 1

        if use_sequential:
            # 🔥 顺序处理（用于 DataLoader worker 环境）
            partial_func = functools.partial(multi_lisa, Rr, fixed_seed, r_min, r_max, beam_divergence,
                                            min_diameter, refractive_index, range_accuracy, a, signal,
                                            density, diameters)
            if self.show_progressbar:
                from tqdm import tqdm
                results = []
                for point in tqdm(point_list, desc="Processing points"):
                    results.append(partial_func(point))
                # 🔥 确保结果数组形状为 (N, 6)
                # 注意：multi_lisa 返回 (x, y, z, i_new, label, intensity_diff)，label 是 int
                results_array = np.array(results)
                if results_array.ndim == 1:
                    results_array = results_array.reshape(-1, 6)
                # 转换为 float32（label 也会被转换为 float，这是预期的）
                pc_new[:, :] = results_array.astype(np.float32)
            else:
                results = [partial_func(point) for point in point_list]
                # 🔥 确保结果数组形状为 (N, 6)
                # 注意：multi_lisa 返回 (x, y, z, i_new, label, intensity_diff)，label 是 int
                results_array = np.array(results)
                if results_array.ndim == 1:
