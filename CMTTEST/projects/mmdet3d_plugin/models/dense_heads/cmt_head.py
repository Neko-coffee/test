            ...
            for view_idx in range(num_views):
                ...
                if img_rgb is not None and img_h is not None and img_w is not None:
                    try:
                        ...
                        # 确保缩放因子是浮点数，并且bias_map是正确的数据类型
                        zoom_factors = (float(img_h) / float(pers_h), float(img_w) / float(pers_w))
                        bias_map_upsampled = zoom(bias_map, zoom_factors, order=1).astype(np.float32)
                        
                        # 显示原始图像
                        ax_left.imshow(img_rgb)
                        
                        # 叠加bias热力图（半透明）
                        # 🔥 只显示增强的前50%（正bias）和抑制的前50%（负bias），中间区域透明
                        from scipy.ndimage import gaussian_filter
                        
                        bias_flat = bias_map_upsampled.flatten()
                        ...
                        if len(positive_bias) > 0:
                            positive_threshold = np.percentile(positive_bias, 50)  # 前50%的阈值
                        else:
                            positive_threshold = 0
                        
                        if len(negative_bias) > 0:
                            negative_threshold = np.percentile(negative_bias, 50)  # 后50%的阈值（更负）
                        else:
                            negative_threshold = 0
                        
                        # 创建mask：只显示前50%的增强和后50%的抑制
                        mask = (bias_map_upsampled > positive_threshold) | (bias_map_upsampled < negative_threshold)
                        
                        # 🔥 美化版可视化参数（只修改样式，保持逻辑不变）
                        SIGMA_BLUR = 8          # 高斯模糊sigma（越大越柔和）
                        ALPHA_MAX = 0.6         # 最大透明度
                        
                        # 🔥 颜色配置：正bias蓝色渐变，负bias红色渐变（与原始RdYlBu一致）
                        # 正bias：深蓝(边缘) -> 浅蓝(中心)
                        POS_COLOR_EDGE = np.array([0.1, 0.2, 0.8])    # 深蓝边缘
                        POS_COLOR_CENTER = np.array([0.6, 0.8, 1.0])  # 浅天蓝中心
                        # 负bias：深红(边缘) -> 浅红(中心)
                        NEG_COLOR_EDGE = np.array([0.8, 0.1, 0.1])    # 深红边缘
                        NEG_COLOR_CENTER = np.array([1.0, 0.7, 0.7])  # 浅粉红中心
                        
                        # 归一化bias值用于colormap
                        bias_max = bias_map_upsampled.max()
                        bias_min = bias_map_upsampled.min()
                        bias_abs_max = max(abs(bias_max), abs(bias_min))
                        
                        if bias_abs_max > 1e-6:
                            # 归一化到[-1, 1]
                            bias_norm = bias_map_upsampled / (bias_abs_max + 1e-8)
                        else:
                            bias_norm = np.zeros_like(bias_map_upsampled)
                        
                        # 分离正bias和负bias的强度（用于渐变）
                        pos_strength = np.clip(bias_norm, 0.0, 1.0)  # [0, 1]
                        neg_strength = np.clip(-bias_norm, 0.0, 1.0)  # [0, 1]
                        
                        # 🔥 高斯平滑（使边缘柔和，减少密集感）
                        pos_smooth = gaussian_filter(pos_strength, sigma=SIGMA_BLUR)
                        neg_smooth = gaussian_filter(neg_strength, sigma=SIGMA_BLUR)
                        
                        # 🔥 创建color map：正bias红色渐变，负bias蓝色渐变
                        pos_color_map = np.zeros((*bias_norm.shape, 3), dtype=np.float32)
                        neg_color_map = np.zeros((*bias_norm.shape, 3), dtype=np.float32)
                        
                        for c in range(3):
                            pos_color_map[:, :, c] = (pos_smooth * POS_COLOR_EDGE[c] + (1.0 - pos_smooth) * POS_COLOR_CENTER[c])
                            neg_color_map[:, :, c] = (neg_smooth * NEG_COLOR_EDGE[c] + (1.0 - neg_smooth) * NEG_COLOR_CENTER[c])
                        
                        # 🔥 合成heatmap：正负区域分别处理
                        heatmap = np.zeros((*bias_norm.shape, 3), dtype=np.float32)
                        
                        # 正bias区域：使用红色渐变（只显示mask区域）
                        pos_mask = (bias_norm > 0) & mask
                        if np.any(pos_mask):
                            heatmap[pos_mask] = pos_color_map[pos_mask]
                        
                        # 负bias区域：使用蓝色渐变（只显示mask区域）
                        neg_mask = (bias_norm < 0) & mask
                        if np.any(neg_mask):
                            heatmap[neg_mask] = neg_color_map[neg_mask]
                        
                        # 🔥 创建alpha通道：mask区域显示，其他区域透明（边缘柔和）
                        alpha_map = np.zeros_like(bias_map_upsampled, dtype=np.float32)
                        alpha_map[mask] = ALPHA_MAX  # mask区域半透明
                        alpha_map = gaussian_filter(alpha_map, sigma=SIGMA_BLUR / 2.0)  # 边缘柔和
                        
                        # 添加alpha通道
                        heatmap_rgba = np.concatenate([heatmap, alpha_map[:, :, np.newaxis]], axis=2)
                        
                        ax_left.imshow(img_rgb)
                        ax_left.imshow(heatmap_rgba, interpolation='bilinear')
                        ax_left.set_title(f'{view_names[view_idx]} - Bias Overlay (Top 50% Red=+, Bottom 50% Blue=-, Iter {iteration})')
                    except Exception as e:
                        # 如果上采样失败，只显示原始图像
