...
class PETRMultiheadAttention(BaseModule):
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        # 🔥 关键修复：处理attn_mask和key_padding_mask的兼容性
        # PyTorch旧版本要求：如果attn_mask是float（加性mask），不能同时传递key_padding_mask
        # 因为attention_bias已经是加性mask，所以当有attention_bias时不传递key_padding_mask
        if attention_bias is not None and final_attn_mask is not None:
            # 有attention_bias时：只使用attn_mask（已包含bias），忽略key_padding_mask
            attn_output, attn_weights = self.attn(
                query=query,
                key=key,
                value=value,
                attn_mask=final_attn_mask,  # 🔥 float类型的加性mask（包含attention_bias）
                key_padding_mask=None,      # 🔥 不传递，避免类型冲突
                need_weights=True
            )
        else:
            # 无attention_bias时：正常传递key_padding_mask
            attn_output, attn_weights = self.attn(
                query=query,
                key=key,
                value=value,
        ...
        if hasattr(self, 'save_attn_weights') and self.save_attn_weights:
            ...
            if attention_bias is not None:
                self.last_attention_bias = attention_bias  # 保存对应的bias
...
class PETRMultiheadFlashAttention(BaseModule):
    ...
    def forward(self,
                ...
                attn_mask=None,
                key_padding_mask=None,
                attention_bias=None,  # 🔥 添加attention_bias参数
                **kwargs):
        """Forward function for `MultiheadAttention`.
        **kwargs allow passing a more general data flow when combining
        with other operations in `transformerlayer`.
        ...
        # 🔥 传递attention_bias到FlashMHA
        out = self.attn(
            q=query,
            k=key,
            v=value,
            key_padding_mask=None,
            attn_bias=attention_bias  # 🔥 传递attention_bias
        )[0]
