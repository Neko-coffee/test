    ...
    def _generate_aqr_attention_bias(self, x, x_img, reference_points, img_metas):
        ...
        try:
            ...
            attention_bias = self.attention_bias_generator(
                ...
                pts_bev=projection_info['pts_bev'],      # [bs, num_queries, 2] BEV坐标
                pts_pers=projection_info['pts_pers'],    # [bs, num_queries, 3] 透视坐标(view, h, w)
                img_metas=img_metas
            )
            ...
            warnings.warn(f"AQR attention bias generation failed: {e}. Using None (no bias).")
            import traceback
            traceback.print_exc()
            return None
